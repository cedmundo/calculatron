...
public class CalculatorController extends Handler implements Serializable {
	...
	private String repetitiveInput;
	private short repetitiveOperation;
	private boolean hitEq;
	...
	public void equalFunction() {
		if (selectedOperation != NEXT_OP_IGN) {
	...
	public void butonPress(int id) {
		...
		case R.id.calculator_button_equalsto:
			if (!hitEq) {
				repetitiveOperation = selectedOperation;
				repetitiveInput = virtualDisplay.toString();
			}
			...
			if (hitEq) {
				history.add(new String[] { virtualDisplay.toString(), "=" });
				this.selectedOperation = repetitiveOperation;
				replaceVirtualDisplay(repetitiveInput);
			}
		...
		if (id != R.id.calculator_button_equalsto)
			hitEq = false;
	}
}
