...
JNIEXPORT void JNICALL Java_com_calculatron_extraction_PendingExtraction_extract(
	...
	jfieldID jfid_neo_connections; // FieldID: NEO_CONNECTIONS_FILE
	...
	jstring js_neo_connections; // String: Where connection file is located.

	const char* jch_file_path; // To call.
	const char* jch_neo_connections; // Where connections file is stored.
	...
	result_string = (char *) malloc(sizeof(char) * 45); // Max 45 character per line.
	...
	// Connections file.
	jfid_neo_connections = env->GetStaticFieldID(jc_pending_extraction,
			"NEO_CONNECTIONS_FILE", "Ljava/lang/String;");

	js_neo_connections = (jstring) env->GetStaticObjectField(
			jc_pending_extraction, jfid_neo_connections);
	...
	double c = (3.5 * avgStd.val[0] / 10) - (avg.val[0] / 100);
	LOGD("C = %lf", c);
	...
	for (; contourLow != 0; contourLow = contourLow->h_next) {
		...
		if (char_bounds.height > 30 && char_bounds.width > 5
			...
			if (aspectRatio <= 1.2) {
				...
				//cvErode(image_segment, image_segment, NULL, 1);
				...
				int32_t digit_category = recognize_number(jch_neo_connections,
						neo_character);
...
JNIEXPORT void JNICALL JNI_OnUnload(JavaVM *vm, void *pvt) {
	LOGD("Releasing static memory for neocognitron...");
	free_register();
	free_static();

	free(global_register());
	free(static_register());
}

JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *pvt) {
	// TODO: Load neocognitron memory block once and free once.
