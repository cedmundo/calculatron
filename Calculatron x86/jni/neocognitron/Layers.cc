...
void Sres(				// response of S-layer
	...
	if (init != 0) {
		nsmax = get1Di_static(NcMAX);
		nsmin = get1Di_static(NcMAX);
		namax = get1Di_static(NcMAX);
		vs2 = get2D_static(NsMAX, NsMAX);
		init = 0;
	}
	for (ic = 0; ic < Nc; ic++) {
...
void Cres(				// response of C-layer
	...
	if (init != 0) {
		ncmax = get1Di_static(NsMAX);
		ncmin = get1Di_static(NsMAX);
		namax = get1Di_static(NsMAX);
		init = 0;
	}
	for (is = 0; is < Ns; is++) {
...
void onoff(				// response of on- & off-center cell layers
	...
	if (init != 0) {
		ncmax = get1Di_static(Ns);
		ncmin = get1Di_static(Ns);
		namax = get1Di_static(Ns);
		init = 0;
	}
	for (is = 0; is < Ns; is++) {
		ncmax[is] = is + ng0;
